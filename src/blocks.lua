...
local function after_place_exchanger(pos, _, _, _)
  core.get_meta(pos):get_inventory():set_size(INPUT_LIST_NAME, 1)
end

local function can_dig_exchanger(pos, _)
  return core.get_meta(pos):get_inventory():is_empty(INPUT_LIST_NAME)
end
...
core.register_node("craft_conflict_exchanger:exchanger", {
  ...
  after_place_node = after_place_exchanger,
  can_dig = can_dig_exchanger,
  on_rightclick = exchangerForm:show_on_node_rightclick(nil, false),
  on_metadata_inventory_put = on_inv_put_exchanger,
  on_metadata_inventory_take = on_inv_take_exchanger,
})
